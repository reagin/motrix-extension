...
export default function App() {
  ...
  const updateConnection = (patch: Partial<StorageSnapshot['connection']>) => {
    setSnapshot((current) => ({ ...current, connection: { ...current.connection, ...patch } }));
  };

  const updateSettings = (patch: Partial<StorageSnapshot['settings']>) => {
            ...
            ? (
                <Section title={t('options.maintenance')} icon={Wrench}>
                  <div className='flex flex-wrap gap-2'>
                    ...
                    <Button variant='outline' onClick={() => void sendRuntimeMessage({ type: 'clear-diagnostics' }).then(refresh)}>
                      <Trash2 />
                      {t('options.clearDiagnostics')}
                    </Button>
                    <Button variant='destructive' onClick={() => void restoreDefaults()}>
                      <RotateCcw />
