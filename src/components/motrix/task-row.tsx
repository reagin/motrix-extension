    ...
    <div data-reveal className="rounded-md border bg-background/65 px-3 py-2">
      ...
      {task.status === 'active' ? (
        <div className="mt-1 flex gap-3 text-[11px] text-muted-foreground">
          <span className="metric-font text-speed-download">↓ {formatSpeed(task.downloadSpeed)}</span>
          <span className="metric-font text-speed-upload">↑ {formatSpeed(task.uploadSpeed)}</span>
        </div>
      ) : null}
